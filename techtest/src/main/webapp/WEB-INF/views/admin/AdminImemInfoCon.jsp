...
<p class="adminP">참가 프로젝트</p>	
	...
	<c:forEach var="hashmap" items="${dataList}">
		<div class="clearFix">
			<div class="col col5_1"></div>
			<div class="col col5_2"><a href="<c:url value='/ProjCon.do?pIdx=${hashmap.PIDX}'/>">${hashmap.PNAME}</a></div>
			<div class="col col5_3">${hashmap.INFUNDS}원</div>
			<div class="col col5_4">${hashmap.INSDATE}</div>
			<fmt:formatNumber value="${hashmap.PNFUNDS}" var="pnFunds" pattern=".00"/>
			<fmt:formatNumber value="${hashmap.PTFUNDS}" var="ptFunds" pattern=".00"/>
			<div class="col col5_5">${(pnFunds / ptFunds) * 100} %</div>
		</div>	  
	</c:forEach>
...
<p class="adminP">QNA 리스트</p> 
	<div class="divTable">  
		...
		<div class="clearFix">
			...
			<a href="<c:url value='/AdminQnaCon.do?bIdx=${hashmap.BIDX}'/>">${hashmap.TITLE}</a>
			
			
			</div>
			<c:choose>
				<c:when test="${hashmap.BDEPTH eq 1}">
				<div class="col col4_3">미답변</div>
				</c:when>
				<c:when test="${hashmap.BDEPTH eq 2 }">
